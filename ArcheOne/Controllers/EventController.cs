    ...
    public class EventController : Controller
    {
        ...
        public IActionResult EventData()
        {
            ...
            try
            {
                ...
                var list = _dbRepo.EventList().Where(x => x.CreatedBy == _commonHelper.GetLoggedInUserId() || x.EventType.ToLower() == "holiday").ToList();
                if (list.Count > 0)
                {
                    foreach (var x in list)
                    {
                        ...
                        getAllEventResModel.start = x.StartDate.Value.Date;
                        getAllEventResModel.end = x.EndDate.Value.Date;
