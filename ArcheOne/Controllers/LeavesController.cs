	...
	public class LeavesController : Controller
	{
		...
		public async Task<CommonResponse> GetPerMonthBalanceAsync()
		{
			...
			try
			{
				...
				DateTime dt2 = new DateTime(DateTime.Now.Year, 01, 01);
				for (DateTime i = dt2; i <= DateTime.Now; i = i.Date.AddMonths(1))
				{

					var BalanceMonth = i.Date.ToString("MMMM");
					var LeaveBalanceList = await _dbRepo.LeaveBalanceLists().Where(x => x.UserId == loginUserId && x.BalanceMonth == BalanceMonth).OrderByDescending(x => x.Id).ToListAsync();
					//var LeaveBalance = await _dbRepo.LeaveBalanceLists().Where(x => x.UserId == loginUserId && x.BalanceMonth == BalanceMonth).ToList().OrderByDescending(x => x.Id).FirstOrDefaultAsync();
					var LeaveBalance = LeaveBalanceList.FirstOrDefault();
					if (LeaveBalance == null)
					{
